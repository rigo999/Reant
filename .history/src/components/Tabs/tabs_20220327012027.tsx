 ...
 */
 ...
 export const Tabs: FC<TabsProps> = (props) => {
    ...
    const navClass = classNames('reant-tabs-nav', {
      'nav-line': type === 'line',
      'nav-card': type === 'card',
    })
    const renderNavLinks = () => {
      return React.Children.map(children, (child, index) => {
        ...
        const classes = classNames('reant-tabs-nav-item', {
    ...
    return (
      <div className={`reant-tabs ${className}`}>
        <ul className={navClass}>
          {renderNavLinks()}
        </ul>
        <div className="reant-tabs-content">
          {renderContent()}
        </div>
      </div>
    )
